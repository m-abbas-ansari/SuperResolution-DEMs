...
class Dataset():
    ...
    def get_batch(self, batch_size, batch_format, indices_only=False):
        ...
        if batch_format =='random_samples':
            while len(indices_batch) < batch_size:
                indices_batch.extend(self.index_queue.get(block=True, timeout=90)) 
            ...
            assert batch_size % num_classes == 0
            while len(indices_batch) < batch_size:
                seed_idx = self.index_queue.get(block=True, timeout=90)
        ...
        features_batch = self.features[indices_batch]
        batch = {
    ...
    def pop_batch_queue(self, timeout=90):
